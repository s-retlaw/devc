...
use devc_core::test_support::TestContainerGuard;
...
async fn test_e2e_runtime_flags() {
    ...
    let _guard = TestContainerGuard::new(rt, rt_prefix, id.0.clone())
        .with_name(config.name.clone().unwrap());
    ...
    // Cleanup (guard handles panic case)
...
async fn test_e2e_parallel_object_commands() {
    ...
    let (rt, rt_prefix) = provider.runtime_args();
    let _guard =
        TestContainerGuard::new(rt, rt_prefix, id.0.clone()).with_name("devc_test_parallel_cmds");
    provider.start(&id).await.expect("start");

...
async fn test_compose_detail_service_navigation() {
    ...
    app.send_key(KeyCode::Char('j'), KeyModifiers::NONE)
        .await
        .unwrap();
    assert_eq!(app.compose_selected_service, 1, "j should move to service index 1");

    // 'j' again moves to the third service
    app.send_key(KeyCode::Char('j'), KeyModifiers::NONE)
        .await
        .unwrap();
    assert_eq!(app.compose_selected_service, 2, "j should move to service index 2");

    // 'k' moves back up
    app.send_key(KeyCode::Char('k'), KeyModifiers::NONE)
        .await
        .unwrap();
    assert_eq!(app.compose_selected_service, 1, "k should move back to service index 1");
    ...
    app.send_key(KeyCode::Char('j'), KeyModifiers::NONE)
        .await
        .unwrap();
    assert_eq!(app.compose_selected_service, 0, "j should wrap around to first service");
}
